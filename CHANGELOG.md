...
- **Port drift after a hot restart**: libupnp is built without `SO_REUSEADDR`; with the control point's connections still in `TIME_WAIT` on the configured port, `bind()` fails and libupnp silently takes port+1 — the control point keeps its cached LOCATION and never reaches the new instance. When a fixed port is configured, `UPnPDevice::start()` now retries every 2 s (up to 75 s) until it gets that port.
