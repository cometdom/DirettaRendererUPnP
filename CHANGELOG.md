...
### Fixed
- **DoP: 3 seconds of PCM silence before audio start** (issue #80 — primary cause of "music + noise" reports). The post-online stabilization in `getNewStream()` computed cycle time as `efficientMTU / bytesPerSecond`. For DoP at 176.4kHz (1056-byte buffers, MTU=1500): 1497 / 1,058,400 × 1e6 = **1.4 µs** cycle time instead of the correct ~1 ms, inflating the required buffer count to 70,000+ (capped at 3000). Result: **3 seconds** of 0x00 PCM silence before any DoP frame reached the DAC. Many DACs only scan for DoP markers at stream initialisation; once they have established a PCM stream they do not re-check mid-stream. Those DACs stayed in PCM mode and played the subsequent DoP frames as regular PCM — music was audible (DSD pulse density is a signal) but with wideband quantisation noise that sounded like white noise. Fix: use `currentBytesPerBuffer` (the actual bytes drained per SDK call) instead of `efficientMTU`. For DoP at 176.4kHz: 1056 / 1,058,400 × 1e6 = 998 µs → ~100 buffers ≈ 100 ms, the intended `DAC_STABILIZATION_MS` value. Same formula fix applied to the DSD branch.
- **DoP: keep frame count even per `pushDSDToDoP` call** (issue #80). The 44.1k-family drift corrector in `getNewStream` alternates between 176 and 177 frames per SDK call to compensate for the non-integer 176400/1000 = 176.4 frames/ms rate. When a 177-frame (1062-byte) drain left exactly 1062 bytes free in the ring, `pushDSDToDoP` wrote ⌊1062/6⌋ = 177 frames — an odd count. `m_dopMarkerState` tracks the DoP marker phase correctly across push boundaries regardless of frame count (odd counts are handled correctly; the next push simply continues from the flipped state), so the odd count does not corrupt the marker stream by itself. The even-frame guard `if (pcmFrames % 2 != 0) pcmFrames--;` is added as a simplifying invariant: each push leaves `m_dopMarkerState` unchanged (net zero flips), which simplifies reasoning about the ring's marker stream and avoids edge cases when free space equals exactly one odd-frame amount. Cost: one unused DoP frame (6 bytes, ≈28 µs) per drift-correction event — less than 0.1% throughput impact. New unit test `test_pushDSD_dop_marker_phase_invariant` verifies the phase behaviour with both even and odd frame counts.
