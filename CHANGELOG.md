...
### Fixed
- **DoP: continuous noise on all tracks** (issue #80, root cause — affects `--dop` and `--dop-msb` equally). The 44.1k-family drift corrector in `getNewStream` alternates between 176 and 177 frames per SDK call to compensate for the non-integer 176400/1000 = 176.4 frames/ms rate. When a 177-frame (1062-byte) buffer is consumed, the ring has exactly 1062 bytes free. `pushDSDToDoP` then writes ⌊1062/6⌋ = **177 frames** — an odd count — which flips `m_dopMarkerState`, causing two consecutive identical markers (0x05/0x05 or 0xFA/0xFA) at the push boundary. The DAC detects the broken alternation and loses DoP sync, playing continuous noise from the very beginning of playback. The 44.1k drift fires approximately every 2–3 ms (40% of drain calls), so the corruption is nearly immediate. Fix: add `if (pcmFrames % 2 != 0) pcmFrames--;` after the free-space cap in `pushDSDToDoP`. This ensures `m_dopMarkerState` always returns to its pre-push value, guaranteeing a perfectly alternating 0x05/0xFA stream in the ring at all times. Cost: one unused DoP frame (6 bytes, ≈28 µs) per drift-correction event — less than 0.1% throughput impact. New unit test `test_pushDSD_dop_marker_phase_invariant` reproduces the failure and verifies the fix.
...
### Fixed
- **CPU affinity: cores silently rejected as invalid on AMD Ryzen with HT/SMT disabled** (reported by Kiran on a Ryzen 7730U running AudioLinux). `sysconf(_SC_NPROCESSORS_ONLN)` returns the *count* of online CPUs (8 on a Ryzen 7730U with HT off), which DRUP incorrectly used as the maximum valid CPU ID. With SMT disabled, Linux takes the odd-numbered logical CPUs offline and keeps the even-numbered ones active (0, 2, 4, ..., 14) — so cores 10, 12, 14 are perfectly valid but were rejected as `>= 8`. The validation silently cleared `config.cpuAudio/Decode/Other`, disabling DRUP's per-thread `pthread_setaffinity_np` calls entirely. Users running AudioLinux's cpuset tuner were partially protected by the slice-level confinement, but DRUP's own finer per-thread pinning (audio on 14, decode on 10, other on 12 independently) was never applied. Fix: new `getOnlineCpus()` helper reads `/sys/devices/system/cpu/online` (which contains the actual list, e.g. `"0,2,4,6,8,10,12,14"`) and validates against set membership instead of a count-based range. Falls back to `0..N-1` if the file is unreadable (containers, older kernels). Error message now shows the real online CPU list instead of the misleading `"cores 0-N"` range. Same fix applied to slim2Diretta v1.4.10.
...
- **`install.sh`: FFmpeg decode test failed on every minimal build** — the test used `-f lavfi -i "sine=..."` which requires the `lavfi` demuxer and `sine` source filter, neither of which is included in the minimal audio-only configure profile. Every minimal build would print a false-positive `[WARNING] FFmpeg decode test failed`. Replaced with a silent `s16le` byte-pipe (`printf '\x00...' | ffmpeg -f s16le -i pipe:0 -f null -`) that works with the minimal profile and tests the actual decode path.
- **`install.sh`: WebUI profiles not refreshed after `--build` / option 3** — `build_renderer()` now detects whether the WebUI is already installed and copies the updated profile JSON files after each build. Previously, profiles were only updated by the explicit `--webui` / option 6 step; a `git pull && ./install.sh --build` left the UI showing stale settings (e.g. DoP option missing) until the user separately ran `--webui`.
