...
Supported CPUs:
  ...
  - ARM aarch64 (e.g. Raspberry Pi 5 / Cortex-A76, no SMT)
EOF
}
...
[Slice]
# Confine the service to the isolated renderer cores. When the user pins
# threads onto a housekeeping core via CPU_AUDIO/CPU_DECODE/CPU_OTHER (e.g.
# CPU_OTHER=0), start-renderer.sh widens this cpuset at runtime to exactly
# 'renderer cores + referenced CPU_* cores' (systemctl set-property), so the
# strict isolation here is preserved whenever no --cpu-* flags are in use.
...
# SCHED_FIFO itself, per thread (Diretta worker at RT_PRIORITY, decode
# thread too when CPU_DECODE is set — otherwise it is SCHED_OTHER now). A
# process-wide FIFO 90 made every other thread (main, libupnp SOAP/SSDP,
# position, log drain, preload) run ABOVE the audio worker, and nice is
# ignored under SCHED_FIFO. Re-run the tuner to apply on an existing host.
