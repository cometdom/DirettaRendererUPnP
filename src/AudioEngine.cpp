...
bool AudioEngine::process(size_t samplesNeeded) {
    ...
    if (samplesRead > 0) {
        ...
        if (m_audioCallback) {
            ...
            if (!continuePlayback) {
                // A Stop/SetURI in flight: DirettaRenderer::stop() already moved
                // us to STOPPED (atomically, before it raised its shutdown flag
                // and the callback bailed) — nothing to tear down, the UPnP
                // thread is doing it. Otherwise the output refused the track
                // (e.g. sink negotiation failed): tear down like any fatal
                // error so the UPnP side sees STOPPED and the idle release
                // timer starts — not a silent PLAYING zombie.
                if (m_state.load() != State::PLAYING) return false;
