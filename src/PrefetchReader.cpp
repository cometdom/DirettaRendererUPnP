...
AVIOContext* PrefetchReader::open(const std::string& url, AVDictionary** options,
    ...
    // Only seekable when the server told us the size (Range requests work on
    // files; live streams would break on a seek attempt anyway).
    m_pb->seekable = (m_size > 0) ? AVIO_SEEKABLE_NORMAL : 0;
...
int PrefetchReader::interruptCb(void* opaque) {
    ...
    if (self->m_interruptCb && self->m_interruptCb->callback) {
...
int PrefetchReader::readFromRing(uint8_t* buf, int bufSize) {
    ...
    while (m_count == 0 && m_error == 0 && !m_stopping) {
        ...
        if (m_interruptCb && m_interruptCb->callback &&
            m_interruptCb->callback(m_interruptCb->opaque)) {
            return AVERROR_EXIT;
        }
    }
