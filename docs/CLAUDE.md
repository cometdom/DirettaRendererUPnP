...
- [x] DoP silence: plain PCM 0x00 for all modes (v2.5.8, issue #80). `fillSilence()` in `getNewStream()` uses plain `memset(0x00)` in all modes including DoP, matching the MinimServer/Asset UPnP pre-encoded DoP reference path that works reliably. An earlier revision generated DoP-marked silence frames (0x69 + alternating markers, tracked by `m_doPSilenceMarkerState`) but this caused persistent "music + constant hiss" on SFORZATO DAC-01 and HOLO Audio — reverted and `m_doPSilenceMarkerState` removed. Verbose mode logs first 5 ring pops with hex-decoded marker+DSD bytes (`[DoP POP #N]`) for field diagnosis.
