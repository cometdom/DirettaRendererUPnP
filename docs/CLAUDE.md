...
- [x] DoP even-frame guard (v2.5.8, issue #80). Keeps each `pushDSDToDoP` call to an even number of frames so `m_dopMarkerState` is unchanged after each push (net-zero flips), simplifying reasoning and avoiding edge cases when free space equals exactly one odd-frame amount. Note: `m_dopMarkerState` correctly handles odd counts across push boundaries; the guard is a simplifying invariant, not a correctness requirement. `if (pcmFrames % 2 != 0) pcmFrames--;` in `pushDSDToDoP`. Unit test `test_pushDSD_dop_marker_phase_invariant` added.
- [x] DoP silence frames during stabilisation (v2.5.8, issue #80 — primary noise cause). `getNewStream()` filled all silence periods with PCM 0x00 (no DoP markers). DACs that detect DoP only at stream init established a PCM stream during silence and never switched to DoP mode → music + wideband quantisation noise. Fix: `fillSilence()` lambda generates `[0x69, 0x69, marker]` DoP silence frames (0x05/0xFA alternating) when `m_isDoPMode` is active. Marker state (`m_doPSilenceMarkerState`) persists across calls; reset on each track open. Applies to all silence paths: prefill, stabilisation, stop, underrun, shutdown.
- [x] DoP stabilisation count fix (v2.5.8, issue #80 — secondary). `getNewStream()` computed cycle time as `efficientMTU / bytesPerSecond` → 1.4 µs at 176.4kHz DoP → 3000 buffers (3 s) instead of 100 ms. Fixed (commit `912a177`) to use `currentBytesPerBuffer`. Also fixes DSD branch and stores missing `m_sampleRate`/`m_bytesPerSample` in `configureRingDSD()`.
