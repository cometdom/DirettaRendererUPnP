...
- [x] DoP marker phase fix (v2.5.8, issue #80 root cause). The 44.1k drift corrector alternated 176/177 frames per SDK call; a 177-frame drain left 1062 bytes free → floor(1062/6)=177 (odd) frames written by `pushDSDToDoP`, inverting `m_dopMarkerState` → consecutive identical markers → DAC loses DoP sync → continuous noise. Fix: `if (pcmFrames % 2 != 0) pcmFrames--;` in `pushDSDToDoP`. Unit test `test_pushDSD_dop_marker_phase_invariant` added.
