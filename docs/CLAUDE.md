...
- [x] IRQ affinity for the target NIC (v2.4.0) — `IRQ_INTERFACE` / `IRQ_CPUS` config keys pin all hardware IRQs (incl. MSI-X queues) of the given NIC to the specified CPU list at service start, walking `/proc/interrupts` and writing each `/proc/irq/N/smp_affinity_list`. Kernel-managed IRQs that refuse reassignment are reported as "skipped". Documented alongside expanded `isolcpus=` kernel cmdline tuning in `docs/CONFIGURATION.md`.
