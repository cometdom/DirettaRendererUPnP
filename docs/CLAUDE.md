...
- [x] DoP continuous noise fix — drift corrector produces odd-frame buffers (v2.5.8, issue #80 — confirmed root cause for yama3kzh/SFORZATO and Dominique/HOLO-via-DDC-0). Two fixes required together: (1) `getNewStream()` drift corrector: in DoP mode, threshold 1000→2000 and add 1→2 frames so all buffers are even (176 or 178 frames at 176.4kHz). Average rate unchanged. (2) `pushDSDToDoP` even-frame guard (`if (pcmFrames % 2 != 0) pcmFrames--;`): ensures ring content is always even×6 bytes so ring→silence transitions always have correct marker phase. With both: `m_doPSilenceMarkerState` and `m_dopMarkerState` never diverge → perfect 0x05/0xFA alternation under all conditions. Commit `8d74693` (getNewStream) + `209365f` (push guard). Unit test `test_pushDSD_dop_marker_phase_invariant` covers the push-side invariant.
- [x] DoP silence frames during stabilisation (v2.5.8, issue #80). `getNewStream()` filled all silence periods with PCM 0x00 (no DoP markers). Fix: `fillSilence()` lambda generates `[0x69, 0x69, marker]` DoP silence frames when `m_isDoPMode` is active. Marker state (`m_doPSilenceMarkerState`) persists across calls; reset on each track open.
