...
- [x] DoP stabilisation silence fix (v2.5.8, issue #80 — primary noise cause). `getNewStream()` post-online stabilisation used `efficientMTU / bytesPerSecond` for cycle time: at 176.4kHz DoP (MTU=1500, 1056-byte buffers) this gave 1.4 µs instead of ~1 ms, inflating buffer count to 3000 (cap) → **3 seconds** of 0x00 PCM silence before any DoP frame. DACs that detect DoP only at stream init stayed in PCM mode → music + wideband noise. Fix: use `currentBytesPerBuffer` → ~100 buffers = 100 ms. Also fixes DSD branch same formula and stores missing `m_sampleRate`/`m_bytesPerSample` in `configureRingDSD()`.
